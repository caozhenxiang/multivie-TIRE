...
def calc_fft(windows, nfft=30, norm_mode="timeseries"):
    ...
    for i in np.arange(2):
        sub_windows = windows[:, i * window_size // 2: (i + 1) * window_size // 2]
        sub_windows = sub_windows - mean_all
        sub_windows_fft = abs(np.fft.fft(sub_windows))
        if i == 0:
            windows_fft = np.expand_dims(sub_windows_fft, axis=-1)
        else:
            windows_fft = np.concatenate((windows_fft, np.expand_dims(sub_windows_fft, axis=-1)), axis=-1)
    # if norm_mode == "window":
    #     windows = windows - mean_per_segment[:, None]
    #     windows_fft = abs(np.fft.fft(windows, nfft))[:, 1:nfft // 2 + 1]
    # elif norm_mode == "timeseries":
    #     windows = windows - mean_all
    #     windows_fft = abs(np.fft.fft(windows, nfft))[:, :nfft // 2 + 1]
    windows_fft = np.mean(windows_fft, axis=-1)
    fft_max = np.amax(windows_fft)
    fft_min = np.amin(windows_fft)
    windows_fft = 2 * (windows_fft - fft_min) / (fft_max - fft_min) - 1
    return windows_fft
...
def compute_wavelet_features(timewindows, wavelet="morl", scale=np.arange(0, 128)):
    if len(timewindows.shape) == 2:
        timewindows = np.expand_dims(timewindows, axis=-1)
    for idx, window in enumerate(timewindows):
        for channel_idx in np.arange(0, window.shape[1]):
            cwtmatr_channel, _ = pywt.cwt(window[:, channel_idx], scale, wavelet)
            if channel_idx == 0:
                cwtmatr = np.expand_dims(cwtmatr_channel, axis=-1)
            else:
                cwtmatr = np.concatenate((cwtmatr, np.expand_dims(cwtmatr_channel, axis=-1)), axis=-1)
        if idx == 0:
            wt_array = np.expand_dims(cwtmatr, axis=0)
        else:
            wt_array = np.concatenate((wt_array, np.expand_dims(cwtmatr, axis=0)), axis=0)
    return norm_windows(wt_array)
