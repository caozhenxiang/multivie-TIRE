...
def create_parallel_AEs(X, enable_summary, seed, ws):
...
def train_model(windows, enable_summary, window_size, seed, verbose=0, nr_epochs=200, nr_patience=20):
    new_windows = prepare_inputs(windows, nr_ae=2, window_size=window_size)
    pae, encoder = create_parallel_AEs(new_windows, enable_summary, seed, window_size)
